...
export const transformDataToMatchExpectedFormat = (rawData: any[]): DetectionResults => {
  const transformedData: DetectionResults = rawData.map((item): ImageAnalysis => ({
    image: {
      id: item.image.id,
    ...
    analysis: {
      ...
      detections: item.analysis.detections.map((detection: { bbox: { x: any; y: any; width: any; height: any; }; confidence: any; }) => ({
        bbox: {
          x: detection.bbox.x,
          y: detection.bbox.y,
